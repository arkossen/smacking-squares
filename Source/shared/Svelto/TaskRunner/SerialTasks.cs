...
namespace Svelto.Tasks
{
	public class SerialTasks: Tasks
	{
		...
		override public IEnumerator GetEnumerator()
		{
			...
			if (debug) Debug.Log("Serialized Tasks Started, number of tasks: " + registeredEnumerators.Count);
			...
			while (registeredEnumerators.Count > 0) 
			{
				...
				while (stack.Count > 0) 
				{
					...
					if (ce is ParallelTasks) //a set of parallel tasks is due to start
						if (debug) Debug.Log ("New Set of Parallel Tasks Started from SerialTasks");
			...
			if (debug) Debug.Log ("All Serialized Tasks Ended");
